...
int main(int argc, char **argv) {
  ...
  bp_zynq_pl *zpl = new bp_zynq_pl(argc, argv);
    ...
    */
    ...
    unsigned long long attaboy_start = zpl->axil_read(0x30 + GP0_ADDR_BASE);
    unsigned long long redirect_start = zpl->axil_read(0x34 + GP0_ADDR_BASE);
    unsigned long long br_cnt_start = zpl->axil_read(0x38 + GP0_ADDR_BASE);
    unsigned long long btb_hit_start = zpl->axil_read(0x44 + GP0_ADDR_BASE);
    unsigned long long bht_hit_start = zpl->axil_read(0x4C + GP0_ADDR_BASE);
    ...
    unsigned long long attaboy_stop = zpl->axil_read(0x30 + GP0_ADDR_BASE);
    unsigned long long redirect_stop = zpl->axil_read(0x34 + GP0_ADDR_BASE);
    unsigned long long br_cnt_stop = zpl->axil_read(0x38 + GP0_ADDR_BASE);
    unsigned long long btb_hit_stop = zpl->axil_read(0x44 + GP0_ADDR_BASE);
    unsigned long long bht_hit_stop = zpl->axil_read(0x4C + GP0_ADDR_BASE);
    ...
    printf("ps.cpp: MPKI                           : %'16.2f\n", (redirect_stop-redirect_start) * 1000.0 / minstrret_delta);
    printf("ps.cpp: BTB hit%%                       : %'16.2f\n", (btb_hit_stop-btb_hit_start) * 100.0 / (attaboy_stop + redirect_stop - attaboy_start - redirect_start));
    printf("ps.cpp: BHT hit%%                       : %'16.2f\n", (bht_hit_stop-bht_hit_start) * 100.0 / (br_cnt_stop-br_cnt_start));
